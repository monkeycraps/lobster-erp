define(function(require, exports, module){
	...
	var FormView = FormInner.View.extend({
		el: '#forminner-mission-6-21', 
		template: _.template($('#template-formview-6-21').html()),
		template_close: _.template($('#template-formview-closed-6-21').html()),
		events: {
		...
		render: function(){
			...
			if( ( app_data.user.role_id == 3 || app_data.user.role_id == 4 ) || this.model.get( 'user_state' ) == 4 || this.model.get( 'state' ) == 2 ){
				...
				if( this.$('.datepicker').length > 0 ){
					this.$('.datepicker').val( this.model.get( 'mail_time' ) ).datepicker({
						language: 'zh-CN', 
						format: 'yyyy-mm-dd'
					})
				}
