...
class RoleModel extends RedBean_SimpleModel {
	...
	static function getCG( $uid ){
		return R::findOne( 'user', 'role_id=?', array( UserModel::ROLE_CG ) );
	}

	static function getDZ( $uid ){
		return R::findOne( 'user', 'role_id=?', array( UserModel::ROLE_DZ ) );
	}
