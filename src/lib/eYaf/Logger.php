...
class Logger extends \SplFileObject {
	...
	public function logQuery($query, $class_name = null, $parse_time = 0, $action = 'Load') {
		...
		$buffer = self::COLOR_SEQ . self::PURPLE . "$class_name $action (" . number_format ( $parse_time * 1000, '4' ) . "ms)  " . self::RESET_SEQ . self::COLOR_SEQ . self::WHITE . $query . self::RESET_SEQ;
	...
	public function logRequest($request) {
		...
		$this->log ( "Processing " . $request->getModuleName () . '\\' . $request->getControllerName () . "Controller#" . $request->getActionName () . " (for {$request->getServer('REMOTE_ADDR')}" . " at " . date ( 'Y-m-d H:i:s' ) . ")" . " [{$request->getMethod()}]" );
		$params = array ();
		$params = array_merge ( $params, $request->getParams (), $request->getPost (), $request->getFiles (), $request->getQuery () );
		$this->log ( "Parameters: " . print_r ( $params, true ) );
	}

	public function logException($exception) {

		$this->log ( get_class ( $exception ) . ": " . $exception->getMessage () . " in file " . $exception->getFile () . " at line " . $exception->getLine () );
